...
public class CanvasViewModel {
    ...
    public void onMouseReleased(final double x, final double y) {
        if (activeTool.get() == ToolType.SELECT) {
            ...
            String jsonData = NetActionSerializer.serializeAction(actionManager.getUndoRedoManager().getLastAction());
        }

        currentPoints.clear();

        // byte[] data = NetActionSerializer.serializeAction(actionManager.getUndoRedoManager().getLastAction()).getBytes();
        // network.sendData(data, dest, module, priority);
    }
    ...
    public void processIncomingMessage(final byte[] data) {
        // Deserialize the data into a NetworkMessage
        String json = new String(data);
        Action action =  NetActionSerializer.deserializeAction(json);
        
        if (action != null) {
            switch (action.getActionType()) {
                case CREATE:
                    actionManager.requestCreate(action.getNewState().getShape());
                    break;
                case MODIFY:
                    actionManager.requestModify(action.getPrevState(), action.getNewState().getShape());
                    break;
                case DELETE:
                    actionManager.requestDelete(action.getPrevState());
                    break;
                default:
                    // Unknown action type
                    break;
    ...
    public AbstractNetworking network;
    public void setNetworking(AbstractNetworking network) {
